...
if( isset( $routes[ $uri ] ) ) {   // у маршрутах є відповідний запис
	...
	//перевіряемо чи э такий контроллер  - [Uri]Controller
	$uri_name = ucfirst($uri); // перша літера переводиться у UpperCase: test -> Test
	$controller_name = "{$uri_name}Controller"; // TestController
	$controller_peth = "./controllers/{$controller_name}.php"; // ./controllers/TestController.php
	if (is_readable($controller_peth)){// є контролер
		include $controller_peth;// означення класу контролера
